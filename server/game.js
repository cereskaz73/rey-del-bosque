...
// Conectividad de un bando, con posibilidad de simular bajas (excludeIds) y un movimiento (moves: id->cellId).
function isConnected(st, color, excludeIds = [], moves = {}) {
  const ps = alive(st, color).filter((p) => !excludeIds.includes(p.id));
...
function movePiece(st, color, pieceId, targetCellId) {
  ...
  if (!isConnected(st, color, [], { [pieceId]: targetCellId })) return err('Ese movimiento rompería la conectividad de tu grupo.');
...
function resolveCombat(st, a, d) {
  ...
  // Regla de máxima prioridad: si las bajas desconectan a cualquier bando, el combate se calcula como empate.
  let connectivityTie = false;
  if (removed.length && (!isConnected(st, 'red', removed) || !isConnected(st, 'blue', removed))) {
